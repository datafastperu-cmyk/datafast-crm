...
| **H-6** | **El tramo entregado hasta el corte no se factura nunca** — la generación excluía a los suspendidos | **CERRADO 09/08.** La generación cobra el TIEMPO ENTREGADO (`diasEntregados` sobre `contratos_historial`). Había DOS generadores; el de producción era el diario, no el mensual |
| **H-7** | **La reactivación exigía deuda TOTAL cero**, no vencida — quien pagaba lo que debía no se reactivaba si tenía una factura emitida sin vencer | **CERRADO 09/08.** No eran dos puertas: eran **CINCO**, y una reactivaba con `automatico = true` saltándose las guardas. Las cinco preguntan ahora a una sola definición |
| **H-8** | Prepago: tras reactivar, **el ciclo en curso no lo emitía nadie** — la generación ya había pasado mientras estaba suspendido | **CERRADO 09/08.** Sin mecanismo nuevo: el generador diario pasó de `===` a `<=` y se auto-repara. La deduplicación por periodo es lo que lo hace seguro |

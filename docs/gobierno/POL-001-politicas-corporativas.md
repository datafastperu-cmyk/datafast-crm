...
|---|---|---|---|---|---|
| **A-1** | **PS-04** — toda consulta filtra por `empresa_id` | **PARCIAL desde 2026-08-08 (ADR-017).** Existe el barrido: de 492 sentencias sobre tablas de empresa, **20 abiertas · 171 transitivas · 13 globales**. **RLS sigue sin ser viable**: la app conecta como SUPERUSER con BYPASSRLS y es dueña de las 111 tablas — activarla no filtraría nada (ver **B-15**) | **Una consulta que omita el filtro devuelve CERO filas.** Requiere primero des-privilegiar el rol de BD | **ADR-017** · bloqueada por **B-15** | **Fuga entre empresas.** Hoy mitigada solo por el contexto: **una sola empresa en la instalación**. Desaparece con el primer cliente multi-empresa |
...
| **B-15** | **PS-01 / OWASP** — mínimo privilegio | **La aplicación se conecta a PostgreSQL como SUPERUSUARIO** (`datafast_db_user`: `rolsuper`, `rolbypassrls`, dueña de las 111 tablas). Cualquier inyección SQL que alcance el motor lo hace con permisos totales: `DROP`, lectura de `pg_authid`, `COPY … TO PROGRAM`. **Y es lo que hace que RLS sea imposible** | Rol de ejecución sin `SUPERUSER` ni `BYPASSRLS`, separado del rol de migración, con `GRANT` mínimo verificado | **Decisión del propietario** (ADR-017 §3.2): el modo de fallo es que al ERP le falte un permiso en producción |
...
|---|---|---|---|---|
| **A — crítico** | **1** — A-1, **parcial**: barrido hecho, RLS bloqueada por B-15 | **3** — A-2 (06/08) · A-3 (07/08) · A-4 (08/08) | **Propietario del producto** | **Requieren fecha comprometida** |

...
| **B-16** | **PC-06** — cero `catch` mudo · **PF-4** (CON-001) — el silencio es el peor modo de fallo | **Encontrado en producción por la Ola 0 (2026-08-16, F-0.1 §5.1.1), no por auditoría del módulo.** `SchemaGuardModule.onApplicationBootstrap()` —la alarma de drift de esquema del Core, corre en cada arranque con `NODE_ENV=production`— llamaba a `schema:log` de TypeORM dentro de un `catch { }` vacío. La causa puntual (`typeorm_metadata` ausente en instalación limpia real, mismo defecto que rompía `npm run db:check`) ya está corregida, y el `catch` **ya registra** el fallo con `logger.error` en vez de tragarlo. Lo que queda abierto es el patrón: **cero cobertura de test** sobre `SchemaGuardModule`, así que nada impide que un `catch` mudo vuelva a escribirse aquí la próxima vez que alguien lo toque, ni que exista el mismo patrón en otro guardián de arranque no revisado todavía. **Segunda ocurrencia encontrada al convertir la Ola 1, grupo 2 (2026-08-16), sin tocarla — registrada, no corregida, por instrucción explícita:** `ProvisionFtthService.marcarUsoTr069()` (`provision-ftth.service.ts:2318`) sella la marca de uso del carril TR-069 con `.catch(() => { /* best-effort */ })` — el comentario declara la intención (no romper la apertura del modal Ver ONU) pero el `catch` no deja ningún rastro observable de que el `UPDATE` falló. Está **correctamente excluida de E03-02/E03-03** (PA-1: es escritura de bookkeeping en el plano de negocio, no una operación de frontera contra hardware), pero el defecto de silencio es el mismo B-16 en otro archivo: si `ftthRepo.update` falla, el operador ve el modal abrir con normalidad y el TTL del carril puede expirarlo sin que nadie lo sepa. **Dos ocurrencias siguen siendo anécdota, no patrón — la tercera sí obligaría al barrido completo de `catch` mudos que el estado objetivo ya preveía** | Test que ejercite `SchemaGuardModule` contra una instalación limpia real y falle si el `catch` no registra, más una revisión del resto de `onModuleInit`/`onApplicationBootstrap` del Core buscando el mismo patrón. `marcarUsoTr069()` queda fuera de ese alcance (no es guardián de arranque) — su corrección espera a que aparezca una tercera ocurrencia y el barrido se vuelva obligatorio | Test de regresión sobre `schema-guard.module.ts` + barrido de guardianes de arranque equivalentes. El barrido de `catch` mudos general (no solo arranque) se abre como tarea propia si aparece la tercera ocurrencia |
...
| **B — riesgo técnico** | **11** | **4** — B-9 **retirada: nunca existió** · B-12 y B-13 (07/08) · **B-14 (08/08)**. Dos nuevas: **B-15**, con su prerrequisito resuelto, y **B-16** (16/08, Ola 0 — causa puntual y silencio ya corregidos, falta el test de regresión; **segunda ocurrencia** registrada 16/08 en `marcarUsoTr069()` durante la Ola 1, sin corregir — dos no son patrón) | Arquitecto · **B-15 escala al propietario** | Con condición de cierre declarada |

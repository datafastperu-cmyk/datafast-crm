...
| **B-4** | **PA-07** — toda mutación de hardware por outbox · **E03-04/PA-19** (E-0.3) — todo comando mutante viaja encolado y persistido | Las operaciones interactivas de `/red/routers` son **síncronas**, sin outbox ni garantías. **Instancia confirmada 2026-08-17 (Ola 1, al investigar antes de tocar el grupo 3b, bloque Firewall/Pppoe/Queue):** `ContratosService.cambiarEstado()` llama `firewallSvc.reactivarCliente()`/`.suspenderCliente()` y `pppoeSvc.setEstado()`/`.desconectarSesion()` **directamente, en línea**, dentro del propio request — el `outboxRed.encolar()` solo se dispara en el `catch` como *fallback* si la llamada síncrona falla, no es el camino primario. Es exactamente lo que E03-04 prohíbe: el comando mutante no viaja encolado y persistido, viaja en la petición HTTP del operador y solo se persiste si el router ya falló una vez | **Toda** mutación pasa por outbox; la operación interactiva devuelve "encolado". La comprobación automática de E03-04 está declarada **Falta** en F-0.1-A (línea `E03-04 \| Comando mutante encolado y persistido \| Test \| Falta`) | **ADR-028** + RDM-001 **R5**. No se resuelve dentro de la Ola 1 (F-0.0 §8) — es rediseño de `cambiarEstado()`, no una conversión de vocabulario |
...
| **B-16** | **PC-06** — cero `catch` mudo · **PF-4** (CON-001) — el silencio es el peor modo de fallo | **PROMOVIDA 2026-08-16 (Ola 1, grupo 2): la tercera ocurrencia apareció y la propia regla que B-16 dejó escrita («dos son anécdota, la tercera obliga al barrido») se cumplió en el mismo lote que la destapó.** Cronología: **(1)** `SchemaGuardModule.onApplicationBootstrap()` —la alarma de drift de esquema del Core, corre en cada arranque con `NODE_ENV=production`— llamaba a `schema:log` de TypeORM dentro de un `catch { }` vacío (encontrado por la Ola 0, F-0.1 §5.1.1; causa puntual y silencio ya corregidos, ahora usa `logger.error`). **(2)** `ProvisionFtthService.marcarUsoTr069()` (`provision-ftth.service.ts:2318`) sella la marca de uso del carril TR-069 con `.catch(() => { /* best-effort */ })` sin dejar rastro observable del fallo — si `ftthRepo.update` falla, el modal Ver ONU abre igual y el TTL del carril puede expirarlo sin que nadie se entere. **(3), la más grave de las tres:** `MensajeriaWorker.procesarNotificacionIndividual()` y `CampanasWorker.procesarCampanaMasiva()` capturaban cualquier fallo de `despachar()` y solo lo logueaban — con `attempts: 3` + backoff **ya configurado** en BullMQ (`workers.constants.ts`, `gateway-monitor.service.ts`) para esos jobs, y sin ejecutarse **nunca**, porque el handler no relanzaba. Una política de reintentos configurada y jamás disparada es peor que no tenerla: alguien la leyó, la dio por vigente, y no lo estaba. Corregido junto con la conversión de `despachar()` a `ResultadoOperacion` (F-0.1 §9.1, grupo 2) — no es una cuarta ocurrencia pendiente, ya se arregló al tocar el código que la tenía. **Variante encontrada en el grupo 3a (mismo día), material del mismo barrido aunque no sea idéntica:** `VpnClienteService.limpiarWizardsAbandonados()` (cron, red de seguridad que libera IPs VPN abandonadas — CLAUDE.md) hace `try { await this.revocar(...) } catch {}`, y `revocarPorToken()` no mira el retorno de `this.revocar(...)`. Antes `revocar()` lanzaba y el `catch {}` al menos tragaba una excepción real; ahora `revocar()` habla `ResultadoOperacion` y NO lanza — un `reintentable`/`indeterminado` (management caído, fs con error) se descarta en silencio en ambos sitios, sin log y sin que el cron sepa que la IP no se liberó. No es regresión de comportamiento (antes era igual de mudo) pero es exactamente el patrón que este barrido existe para encontrar — **no se corrige en este lote** | **Alcance ampliado de "test sobre un módulo" a barrido general**: localizar todo `.catch(() => {})` / `catch {}` sin `logger.*` ni relanzamiento **y todo retorno de `ResultadoOperacion` descartado sin comprobar la clase** en `backend/src/`, clasificar cada uno (guardián de arranque / bookkeeping de negocio / mecanismo de reintento configurado y muerto — la categoría (3) es la de mayor riesgo y se prioriza), y para cada uno: o se corrige (log como mínimo), o se documenta aquí por qué el silencio es intencional. **`marcarUsoTr069()`, `limpiarWizardsAbandonados()` y `revocarPorToken()` entran en el barrido** — ninguno queda fuera de alcance | **INVENTARIO EJECUTADO 2026-08-17** (`docs/gobierno/inventario/POL-001-B16-barrido-catch-mudo.md`): 106 sitios (`catch {}` 23 + `.catch(() => {})` 83), clasificados por módulo. `limpiarWizardsAbandonados()` (`vpn-cliente.service.ts:460`) confirmado como el caso ya nombrado arriba — su `catch{}` ya no atrapa nada real desde que `revocar()` habla `ResultadoOperacion`. 5 sitios adicionales marcados de revisión prioritaria (credenciales cifradas de mensajería, lectura de licencia, parseo de IPs VPN en uso). **Deliberado, no un olvido: el inventario es solo lectura — clasificar cuáles se corrigen y en qué ola es una decisión aparte, todavía sin tomar.** La mitad del alcance sobre `ResultadoOperacion` descartado sin comprobar clase **sigue sin ejecutarse** — es un barrido semántico distinto, de tamaño comparable, registrado como continuación pendiente. Falta además el test de regresión sobre `schema-guard.module.ts` |
...
| **B — riesgo técnico** | **11** | **4** — B-9 **retirada: nunca existió** · B-12 y B-13 (07/08) · **B-14 (08/08)**. Dos nuevas: **B-15**, con su prerrequisito resuelto, y **B-16** (16/08, Ola 0 — causa puntual y silencio ya corregidos). **PROMOVIDA 16/08**: tercera ocurrencia del mismo patrón (workers de cola con `attempts:3` configurado y nunca ejecutado) cumplió la regla «dos son anécdota, la tercera obliga al barrido» — alcance pasa de un test puntual a un barrido general de `catch` mudo en todo el Core, registrado, aún sin ejecutar | Arquitecto · **B-15 escala al propietario** | Con condición de cierre declarada |

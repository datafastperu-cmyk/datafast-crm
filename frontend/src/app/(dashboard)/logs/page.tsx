...
import { Terminal, Search, Filter, Download, RefreshCcw, AlertCircle, Info, AlertTriangle, Bug } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description="Registro de eventos, errores y actividad del sistema"
        breadcrumbs={[{ label:'Logs del Sistema' }]}
        actions={
          <div className="flex gap-2">
            <button
              onClick={() => setAuto(a => !a)}
              className={cn('flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors',
                auto ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-muted text-muted-foreground border border-border'
              )}>
          ...
          <div className="flex gap-1 bg-muted rounded-lg p-0.5">
            {(['todos','info','warning','error','debug'] as const).map((v) => (
              <button key={v} onClick={() => setNivel(v)}
                className={cn('text-xs px-2.5 py-1.5 rounded-md capitalize transition-colors',
          ...
          <table className="w-full border-collapse">
            ...
            <tbody className="font-mono">
              {filtered.map((log) => {
                ...
                return (
                  <tr key={log.id}
                      className={cn('border-b border-border/40 hover:bg-muted/20 transition-colors group',

...
export class GoogleOAuthService implements OnModuleInit {
  ...
  // Limpia la fila huérfana de BD cuando el wizard se cancela sin completar OAuth.
  // Las credenciales de app (process.env / .env.production) se conservan intencionalmente
  // para que el usuario no deba re-ingresarlas si reintenta el wizard.
  async cancelarSetup(empresaId: string): Promise<void> {
    ...
    if (account && account.status !== GoogleSyncStatus.CONNECTED) {
      ...
      this.logger.log(`[${empresaId}] Setup wizard cancelado — fila huérfana eliminada`);
    }
  }

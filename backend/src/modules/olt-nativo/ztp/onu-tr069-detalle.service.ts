...
export class OnuTr069DetalleService {
  ...
  // NOTA VIO: "aceptado ≠ materializado". Encolar el task solo confirma "aceptado". La
  // MATERIALIZACIÓN (la ONU reinició de verdad) se verifica con una lectura INDEPENDIENTE del
  // plano operativo — el "Last up time" de la OLT (display ont info), NO el uptime de GenieACS
  // (que queda rancio y da falsos negativos). Esa verificación la hace el frontend contra el
  // endpoint `onu/:contratoId/olt-estado` (ProvisionFtthService), que sí tiene acceso a la OLT.
  async reboot(serial: string): Promise<{ ok: boolean; mensaje: string }> {
    ...
    return { ok: true, mensaje: `Reinicio ACEPTADO por la ONU ${serial} — verificando materialización (debe reiniciar en ~1-3 min).` };
  }

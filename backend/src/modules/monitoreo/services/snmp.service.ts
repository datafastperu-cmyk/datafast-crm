...
export class SnmpService {
  ...
  async getSystemInfo(
    host: string, community = 'public', version = 2, extended = false,
  ...
  async getCpuMemory(
    host: string, community = 'public', version = 2,
  ): Promise<{ cpu: number | null; memory: number | null }> {
  ...
  async getTraficoInterfaz(
    host: string, community: string, ifIndex: number, version = 2,

...
export class SistemaService {
  ...
  triggerUpdate(): void {
    ...
    const script = [
      ...
      '# pg_dump nativo si existe; si no, vía contenedor Docker de Postgres',
      'if command -v pg_dump > /dev/null 2>&1; then',
      '  PGPASSWORD="$DBPASS" pg_dump -h "$DBH" -p "$DBP" -U "$DBU" "$DBN" | gzip > "$BK_SQL" || fail_abort "pg_dump falló"',
      'else',
      '  PG_CTN=$(docker ps --format "{{.Names}} {{.Image}}" 2>/dev/null | awk \'/postgres/{print $1; exit}\')',
      '  [ -n "$PG_CTN" ] || fail_abort "no hay pg_dump ni contenedor Postgres para respaldar"',
      '  docker exec -e PGPASSWORD="$DBPASS" "$PG_CTN" pg_dump -U "$DBU" "$DBN" | gzip > "$BK_SQL" || fail_abort "pg_dump (docker: $PG_CTN) falló"',
      'fi',
    ...
    const items = await this.ds.query(
      `SELECT nl.id, nl.contrato_id, nl.cliente_id, nl.empresa_id, nl.telefono, nl.canal,
              ...
              COALESCE(
                ...
                cl2.nombre_completo,
                NULLIF(TRIM(CONCAT_WS(' ', cl2.nombres, cl2.apellido_paterno, cl2.apellido_materno)), ''),
                nl.variables->>'clienteNombre',
                em.razon_social,
                'DATAFAST'
              ) AS cliente_nombre
       FROM notificaciones_logs nl
  ...
  async eliminarNotifLog(logId: string, empresaId: string): Promise<void> {
    const [log] = await this.ds.query(`
      SELECT id FROM notificaciones_logs
      WHERE id = $1 AND empresa_id = $2
    `, [logId, empresaId]);
